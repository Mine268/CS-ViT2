...
# Agent 重要注意事项
1. 重要，修改代码之前行程计划与用户讨论，严禁直接上手改代码
2. 每一次回答之后加一句 GG
3. 对于每一次对话中的数据格式、模型调用、使用习惯、需要注意的事项等内容，如有必要请将其保存在 `docs/` 文件夹中，以便在后续需要使用的时候可以快速查阅，提升效率。同时对应的代码如果出现了修改请同步更新 `docs/` 中的文档。为了快速查阅，在编写文档的时候开头可以简短概括改文档的内容，同时还应该创建一个快速导读的文档作为总览，并实时保持更新
4. 有问题就问，是怎么样就怎么说，不要迎合用户