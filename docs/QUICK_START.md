# 快速开始 - 渐进式Dropout训练

## 🎯 核心改进

### 问题
原训练配置在早期阶段过拟合加剧，验证波动大

### 解决方案
1. **渐进式Dropout**: 训练前10000步禁用dropout，后期启用提升泛化
2. **验证集固定seed**: 确保每次验证使用相同数据，便于对比

## 🚀 立即开始

### 1. 验证代码正确性
```bash
python -m tests.test_progressive_dropout
```

**预期输出**:
- ✅ Dropout计算函数测试通过
- ✅ 模型接口测试通过
- ✅ 所有Dropout层正确更新

### 2. 启动训练
```bash
# Stage 1 训练（使用默认配置，推荐）
python script/train.py --config-name=stage1-dino_large

# 或自定义参数
python script/train.py --config-name=stage1-dino_large \
    GENERAL.dropout_warmup_step=8000 \
    MODEL.handec.dropout=0.15

# Stage 2 训练（自动加载 Stage 1 权重）
python script/train.py --config-name=stage2-dino_large
```

### 3. 监控训练

**日志输出**:
```
3000/100000 lr=1.0000e-04 dropout=0.000 total=1.15 loss_theta=0.08 ...
                              ↑ dropout率监控
```

**AIM Dashboard**:
- 查看 `dropout_rate` 曲线
- 对比训练loss vs 验证loss
- 关注验证MPJPE波动

## 📋 关键配置

### config/stage1-dino_large.yaml
```yaml
GENERAL:
  dropout_warmup_step: 10000  # Dropout预热步数
  val_seed: 42                # 验证集固定seed

MODEL:
  handec:
    dropout: 0.1              # 目标dropout率
```

## 📊 预期效果

### 训练早期 (0-10000步)
- Dropout = 0.0 → 训练loss更低
- 过拟合gap减小: +57.5mm → ~+45mm
- 优化更稳定

### 训练中后期 (10000+步)
- Dropout = 0.1 → 泛化性提升
- 验证波动减小: 22.41mm → <18mm
- 指标更一致

### 整体指标预期
- **平均MPJPE**: 84.47mm → <80mm (-5%+)
- **标准差**: 22.41mm → <18mm (-20%+)
- **训练loss**: 1.230 → ~1.10 (-10%+)

## 🔍 故障排查

### 问题: 测试失败
```bash
# 检查环境
python -c "import torch; import transformers; print('OK')"

# 检查模型文件
ls model/facebook/dinov2-large/
```

### 问题: 训练loss不降
- 检查dropout_warmup_step是否设置正确
- 查看日志中的dropout值是否符合预期
- 确认学习率调度正常

### 问题: 验证集指标不稳定
- 确认val_seed已设置
- 检查WebDataset路径是否正确
- 增加验证频率以获得更多数据点

## 📁 重要文件

### 文档
- [IMPROVEMENTS_SUMMARY.md](IMPROVEMENTS_SUMMARY.md) - 详细改进说明
- [CODE_DESIGN.md](CODE_DESIGN.md) - 代码设计文档
- [REFACTORING_SUMMARY.md](REFACTORING_SUMMARY.md) - 重构总结

### 代码
- `../src/utils/train_utils.py` - 工具函数
- `../src/model/net.py` - 模型接口
- `../script/train.py` - 统一的 Stage 1/2 训练脚本

### 测试
- `../tests/test_progressive_dropout.py` - 测试脚本

## 🎓 理论依据

### 为什么需要渐进式Dropout?

1. **DINOv2预训练**: 特征已经很鲁棒，早期不需要额外正则化
2. **Fine-tuning稳定性**: Dropout干扰会阻碍backbone适应新任务
3. **后期泛化**: 数据拟合稳定后，dropout防止过拟合

### Dropout预热曲线
```
Dropout率
  0.10 ┤                  ┌────────────
       │                  │
  0.05 ┤                  │
       │                  │
  0.00 ┤──────────────────┘
       └──────┬───────────┬──────────→ Step
           10000       50000
       ↑ 预热期      ↑ 正常训练
```

## 🔗 快速链接

- AIM Server: `aim://10.208.33.93:53800`
- Checkpoint目录: `checkpoint/`
- 数据集: `/mnt/qnap/data/datasets/webdatasets/`

## ✅ 检查清单

训练前:
- [ ] 运行`python -m tests.test_progressive_dropout`确认代码正确
- [ ] 检查配置文件中的`dropout_warmup_step`和`val_seed`
- [ ] 确认AIM服务器可访问
- [ ] 清理旧的checkpoint（可选）

训练中:
- [ ] 监控dropout率变化
- [ ] 对比训练/验证loss
- [ ] 记录关键step的验证MPJPE

训练后:
- [ ] 绘制训练曲线对比
- [ ] 计算验证指标统计（均值、标准差）
- [ ] 对比修改前后的效果

---

**准备就绪！开始训练:**
```bash
# Stage 1 训练
python script/train.py --config-name=stage1-dino_large

# Stage 2 训练（需要先完成 Stage 1）
python script/train.py --config-name=stage2-dino_large
```

**注**: `script/train.py` 是统一的训练脚本，支持 Stage 1 和 Stage 2。旧的 `script/stage1.py` 已重命名为 `train.py`。

**GG**
