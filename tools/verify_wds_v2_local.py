"""
Local visual verification tool for the project's V2 WebDataset pipeline.

Compared with the external reference verifier, processed MANO rendering always uses
right-hand MANO because preprocessing canonicalizes all hands into right-hand space.
"""
...
def _save_metadata(batch: Dict[str, List], output_dir: str, bx: int, tx: int) -> None:
    metadata = {
        ...
        "flip": bool(batch["flip"][bx]) if "flip" in batch else None,
...
def verify_origin_sample(batch: Dict[str, List], output_dir: str, bx: int, tx: int) -> None:
    ...
    joint_hand_origin = batch.get("joint_hand_origin", batch["joint_hand_bbox"])[bx, tx].cpu().numpy()
...
def verify_processed_sample(
    ...
    joint_patch_origin = batch.get("joint_patch_origin", batch["joint_patch_bbox"])[bx, tx].cpu().numpy()
    ...
    if has_intr and has_mano and smplx is not None:
        right_mano = smplx.create("model/smplx_models", "mano", use_pca=False, is_rhand=True)
        mano_pose = batch["mano_pose"][bx, tx][None].cpu()
        mano_shape = batch["mano_shape"][bx, tx][None].cpu()
        focal = batch["focal"][bx, tx].cpu()
        princpt = batch["princpt"][bx, tx].cpu()
        with torch.inference_mode():
            mano_output = right_mano(
                betas=mano_shape,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Local visual verification for the project's V2 pipeline")
    parser.add_argument("--urls", nargs="+", required=True, help="Tar path patterns")
    parser.add_argument("--output-dir", default="temp_verify_v2_local")
    ...
    parser.add_argument("--persp-rot-max", type=float, default=0.0)
    parser.add_argument("--joint-rep-type", default="3")
    parser.add_argument("--perspective-normalization", action="store_true")
...
def main() -> None:
    ...
    batch_processed, trans_2d_mat, _ = preprocess_batch(
        ...
        device,
        pixel_aug=None,
        perspective_normalization=args.perspective_normalization,
    )
